...
void joystick_thread_fn(const char* devnode = "/dev/input/event5") {
    ...
    // poll()’s 3rd arg is a timeout in ms. With 200 ms we periodically wake up
...
#ifdef CALIBRATION_MODE
    std::thread joys(joystick_thread_fn,"/dev/input/event5");
#endif
