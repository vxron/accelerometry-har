#!/usr/bin/env python3 
"""
Train two activity SVMs (static and dynamic) with feature selection and ONNX export.
...
(6) Trains two SVMs (RBF kernels): activity_svm_static and activity_svm_dynamic
    - Feature selection: SelectKBest(Mutual Information) -> correlation filter -> SFS wrapper
...
AR_ORDER = 4    # for number of autoregression coefficients
RNG      = 13   # random seed: keep constant for reproducibility across trials on same training set
np.random.seed(RNG)

# feature-selection hyperparameter defaults
"""
FeatureSelector runs in three stages inside each pipeline (static/dynamic)...
1. Mutual Information filter (SelectKBest) -> keep top k_best features in terms of MI
2. Correlation filter -> drop features that are too correlated to preserve iid
3. Sequential Feature Selection (SFS) -> from what remains, greedily add up to sfs_k features that most improve CV accuracy (wrapper step)
-> 'Greedily' means SFS adds one feature at a time (the one that currently gives the largest improvement in CV accuracy, and never revisits or replaces earlier picks "sequential")
-> Fast but not guaranteed to find the absolute best subset (just good enough)
"""
# These are just defaults - altered by nested_cv_train_category which tests variations of them for optimal accuracy [see param_grid]
K_BEST_STATIC  = 12 # number of top features to retain after MI
K_BEST_DYNAMIC = 18 # dynamic activities typically need more features
SFS_K_STATIC   = 3  # number of features to be added by the wrapper (SFS)
SFS_K_DYNAMIC  = 4
...
def _selector_summary(pipeline: Pipeline) -> Dict:
    ...
    try:
        ...
        if hasattr(sel, "mi_scores_") and sel.mi_scores_:
            # only report MI for the final selected ones
...
def generate_report(
    ...
    # describe the CV splitter (build cv_desc)
    try:
    ...
    # list of selected features (fallback if missing)
...
def build_windows(df: pd.DataFrame, subject_id: str, start_activity: str = "") -> List[Dict]:
    ...
    labels_series = assign_labels_to_blocks(len(active), blocks, order=ACTIVITY_ORDER)
        ...
        for s, e in windows_from_block(a, b, WIN, HOP, GUARD):
            if (labels_series[s:e] == lab).all():
                windows.append({
                    ...
                    "block_id": block_id, # local id per file (0, 1, 2...)
                    "block_uid": f"{subject_id}#blk{block_id}" # global id that combines subject + block_id
                })
    return windows

# General model: windows are from all csv data in data_dir (across subjects)
...
# "Per person" model: windows are from all csv data within data_root/person folder
...
# Preprocessing
# ------------------------------------------------------------------------
# pre-denoising: clip extreme spikes in a window down to reasonable cutoff, e.g. [0.2, 0.5, 9.0] w/ cutoffs [-2,+2] -> output [0.2,0.5,2]
def robust_mad_clip(arr: np.ndarray, k: float = 10.0) -> np.ndarray:
    if arr.size == 0:
        return arr
    med = np.median(arr)
    mad = np.median(np.abs(arr - med)) # median absolute deviation 
    cutoff = k * (mad if mad > 1e-9 else np.std(arr) + 1e-9) # fallback to std if mad isn't meaningful (flat data)
    lo, hi = med - cutoff, med + cutoff # clip extrema that are way more or less than mad cutoff 
    return np.clip(arr, lo, hi)

# Highpass 2nd order butterworth IIR filter with cutoff of 0.5Hz to remove gravity & very slow drift / offsets 
# Butterworth has maximally flat passband; 2nd order transitions smoothly
def highpass_butter(x: np.ndarray, fs: float, fc: float = 0.5, order: int = 2) -> np.ndarray:
    ...
    if s.size < max(8, nmin):
        return s - np.mean(s) # too short for IIR -> use "first differences" surrogate
    s = np.nan_to_num(s, nan=0.0, posinf=0.0, neginf=0.0)
    wn = fc / (0.5 * fs)
    wn = float(min(max(wn, 1e-5), 0.999))
    try:
        b, a = butter(order, wn, btype='highpass')
        return filtfilt(b, a, s, method="gust") # filtfilt avoids phase distortion (zero phase filter)
    except Exception:
        s0 = s - np.mean(s)
        try:
...
def preprocess_window(x: np.ndarray, y: np.ndarray, z: np.ndarray,
    ...
    # (1) MAD: Extrema clipping with median absolute deviation based cutoff from median value
    ...
    # (2) Median Filter for impulse noise removal / smoothing
    if denoise:
        k = 3 if len(x) >= 3 else 1 # each output sample = median of itself +/- 1 neighbours (k=3 total) -> guard for shorter signals
        # ^increase k for more smoothing
        if k % 2 == 0: k += 1 # medfilt only talks odd inputs
        x = medfilt(x, kernel_size=k)
        y = medfilt(y, kernel_size=k)
        z = medfilt(z, kernel_size=k)
    # (3) Butterworth highpass for DC removal
...
def bandpower_welch(sig: np.ndarray, fs: float, nperseg: int = 128):
...
# Compute all static features (skewness, kurtosis, ar_res_var, etc) and save to dictionary
...
# Compute all dynamic features and save to dictionary
def feature_dynamic_core(x: np.ndarray, y: np.ndarray, z: np.ndarray, fs: float) -> Tuple[List[float], List[str]]:
    ...
    f, Pxx = bandpower_welch(mag, fs=fs, nperseg=nperseg)
...
# Tries different combinations of features to select the best subset of those calculated
class FeatureSelector(BaseEstimator, TransformerMixin):
    def __init__(self, k_best=10, sfs_k=3, corr_thr=0.90,
                 C=4.0, gamma='scale', cv_sfs=3, random_state=RNG):
        self.k_best = k_best
        self.sfs_k = sfs_k
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, **fit_params):
        # ----- names (handle PCA upstream changing dimensionality) -----
        ...
        self.mi_scores_ = {
            self._orig_names[i]: (None if np.isnan(self.mi_scores_vec_[i]) else float(self.mi_scores_vec_[i]))
            for i in mi_idx
        }
        names_mi = [self._orig_names[i] for i in mi_idx]

        # ----- (2) correlation filter (remove near-duplicates) -----
        X_unc, names_unc, kept_rel = corr_filter(X_mi, names_mi, thr=self.corr_thr)
        kept_global = [mi_idx[i] for i in kept_rel]

        # If nothing left or SFS disabled, finalize here
        if X_unc.shape[1] == 0 or self.sfs_k == 0:
            ...
            return self
                ...
                # ----- (3) SFS: use precomputed CV folds if provided (block-stratified) -----
        sfs_cv     = fit_params.get("sfs_cv", None)      # may be a list of (tr_idx, va_idx)
        sfs_groups = fit_params.get("sfs_groups", None)  # unused in precomputed mode

        # If we don't have at least 2 classes, skip SFS
        ...
        # Build a CV to use inside cross_val_score:
        # - If sfs_cv is a list/tuple of (train,val) index pairs → use it directly.
        # - Else fall back to safe StratifiedKFold.
        use_precomputed = isinstance(sfs_cv, (list, tuple)) and len(sfs_cv) > 0 and isinstance(sfs_cv[0], (list, tuple))
        if use_precomputed:
            cv_for_sfs = sfs_cv  # precomputed folds over the CURRENT training slice
            needs_groups = False
        else:
            from sklearn.model_selection import StratifiedKFold
            cap = int(pd.Series(y).value_counts().min())
            if cap < 2:
                self.selected_names_   = names_unc
                self.selected_indices_ = kept_global
                self.sfs_selected_names_order_ = self.selected_names_.copy()
                return self
            k = max(2, min(int(self.cv_sfs or 3), cap))
            cv_for_sfs = StratifiedKFold(n_splits=k, shuffle=True, random_state=self.random_state)
            needs_groups = False

        selected_rel, remaining = [], list(range(X_unc.shape[1]))
        self.sfs_selected_names_order_ = []

        def _cv_score(cols_rel: List[int]) -> float:
            if not cols_rel:
                return 0.0
            Xs = X_unc[:, cols_rel]
            est = SVC(kernel='rbf', C=self.C, gamma=self.gamma, class_weight='balanced')
            return float(np.mean(cross_val_score(est, Xs, y, cv=cv_for_sfs, n_jobs=-1)))

        target_k = min(self.sfs_k, X_unc.shape[1])
        while len(selected_rel) < target_k and remaining:
            best_j, best_sc = None, -1.0
            for j in remaining:
                sc = _cv_score(selected_rel + [j])
                if sc > best_sc:
                    best_sc, best_j = sc, j
...
# StandardScaler (implicit inside selector) -> optional PCA -> FeatureSelector -> SVM.
...
def make_block_stratified_folds(y: np.ndarray,
                                block_ids: np.ndarray,
                                desired_max_splits: int = 5,
                                strict_test_two_classes: bool = True
                                ) -> list[tuple[np.ndarray, np.ndarray]]:
    """
    Build CV folds by splitting at the BLOCK level (each block has a single label),
    then expand to window indices. Guarantees no block leakage across folds.

    strict_test_two_classes:
      - True  -> both TRAIN and TEST must have ≥2 classes (use for INNER CV)
      - False -> TRAIN must have ≥2 classes; TEST may be single-class (use for OUTER CV)
    """
    y = np.asarray(y)
    block_ids = np.asarray(block_ids)

    # Unique blocks with their single class label (each block = 1 label)
    df = pd.DataFrame({"b": block_ids, "y": y})
    b_unique = df.drop_duplicates("b")[["b", "y"]].reset_index(drop=True)
    blocks = b_unique["b"].to_numpy()
    b_labels = b_unique["y"].to_numpy()

    # Capacity is limited by the smallest class's number of blocks
    per_class_blocks = pd.Series(b_labels).value_counts()
    cap = int(per_class_blocks.min()) if not per_class_blocks.empty else 0
    if cap < 2:
        raise ValueError("Not enough blocks per class to form 2 folds. Record more blocks.")

    k = max(2, min(int(desired_max_splits), cap))

    skf = StratifiedKFold(n_splits=k, shuffle=True, random_state=RNG)
    folds = []

    # Map block -> window indices
    idx_by_block = {b: np.nonzero(block_ids == b)[0] for b in blocks}

    for tr_bi, va_bi in skf.split(np.zeros_like(blocks), b_labels):
        tr_blocks = set(blocks[tr_bi])
        va_blocks = set(blocks[va_bi])
        tr_idx = np.concatenate([idx_by_block[b] for b in tr_blocks])
        va_idx = np.concatenate([idx_by_block[b] for b in va_blocks])

        # TRAIN must have ≥2 classes to fit a classifier; TEST condition depends on strictness
        if np.unique(y[tr_idx]).size < 2:
            continue
        if strict_test_two_classes and np.unique(y[va_idx]).size < 2:
            continue

        folds.append((np.sort(tr_idx), np.sort(va_idx)))

    if len(folds) < 2:
        # fall back to k=2 once
        if k != 2:
            return make_block_stratified_folds(
                y, block_ids, desired_max_splits=2, strict_test_two_classes=strict_test_two_classes
            )
        raise ValueError("Could not build valid block-stratified folds under constraints.")

    return folds

# STRAT LIMITS FOR WINDOW LEVEL
def _safe_k_for_strat(y: np.ndarray, desired: int) -> int:
    """
    Decides how many folds K we can *safely* run for StratifiedKFold at the WINDOW level.
    (i.e. randomizing CV folds purely by window)
    Rule: k cannot exceed the smallest class count; otherwise some folds would end up missing that class.
    """
    import pandas as pd, numpy as np
    vc = pd.Series(y).value_counts()
    cap = int(vc.min()) if not vc.empty else 0
    return max(2, min(desired, cap)) if cap >= 2 else 0

# STRAT LIMITS FOR BLOCK LEVEL
def _blocks_per_class_ok(y: np.ndarray, block_ids: np.ndarray, desired_k: int) -> bool:
    """
    Check if *block-level* CV with K folds is possible.
    (i.e. randomizing CV folds by block such that one fold doesn't see windows from the same block as another fold)
    Return True if every class has at least `desired_k` distinct blocks (e.g. stand blocks = 3, sit blocks = 2, could only stratify with k = 2)
    """
    import pandas as pd
    df = pd.DataFrame({"y": y, "b": block_ids})
    per = df.drop_duplicates(["y", "b"]).groupby("y")["b"].nunique()
    return (not per.empty) and int(per.min()) >= desired_k

def nested_cv_train_category(
    X: np.ndarray,
    y: np.ndarray,
    groups: np.ndarray,                 # BLOCK IDs
    ...
    """
    - We run a nested Cross Validation (outer loop to estimate generalization, inner loop for model selection)
    - We PREFER block-level CV (no window leakadge across blocks), but only if feasible (i.e. enough data to stratify by block)
    - Otherwise we fall back to window-level CV (stratifiedKfold on windows; ignore blocks and simply ensure each fold has similar class proportions by sampling individual windows)
    - OUTER CV: splits data into k folds -> trains on k-1 folds -> evaluates on the held-out outer fold -> avg outer-fold accuracies to get unbiased generalization score
    - INNER CV: on each outer-train slice, perform a GridSearchCV to pick hyperparameters (PCA on/off, SVM C/gamma, etc) -> best estimator from inner search is what's evaluated on outer test fold
    """
    y = np.asarray(y).reshape(-1)
    if np.unique(y).size < 2:
        raise ValueError(f"[{label}] Need ≥2 classes.")

    if groups is None or groups.size != y.size:
        # treat as single block if missing
    ...
    if fast:
        param_grid_common = {
            ...
            "svm__gamma":         ["scale", 0.02, 0.05, 0.1],
        }
    else:
        param_grid_common = {
            "pca": ['passthrough', PCA(n_components=0.95, svd_solver='full', random_state=RNG)],
    ...
    desired_outer = 5
    outer_desc = ""
    if _blocks_per_class_ok(y, groups, 2):
        folds = make_block_stratified_folds(
            y, groups, desired_max_splits=desired_outer, strict_test_two_classes=False
        )
        if len(folds) >= 2:
            outer_cv = folds
            outer_desc = f"Block-StratifiedKFold(n_splits={len(folds)})"
        else:
            outer_cv = None
    else:
        # Window-level fallback
        k = _safe_k_for_strat(y, desired_outer)
        if k >= 2:
            outer_cv = StratifiedKFold(n_splits=k, shuffle=True, random_state=RNG)
            outer_desc = f"Window-StratifiedKFold(n_splits={k})"
        else:
            raise ValueError(f"[{label}] Not enough samples per class to run CV.")

    print(f"[{label}] OUTER policy: {outer_desc}")

    # ---------------- Outer loop ----------------
    outer_scores: List[float] = []
    last_best_params: dict = {}

    # Normalize to an iterable of (train_idx, test_idx)
    if isinstance(outer_cv, StratifiedKFold):
        folds_iter = list(outer_cv.split(X, y))
    else:
        folds_iter = outer_cv  # already list of (tr,te)

    for fold_id, (tr_idx, te_idx) in enumerate(folds_iter, 1):
        ...
        desired_inner = 3
        inner_desc = ""
        if _blocks_per_class_ok(y_tr, g_tr, 2):
            inner_folds = make_block_stratified_folds(
                y_tr, g_tr, desired_max_splits=desired_inner, strict_test_two_classes=True
            )
            if len(inner_folds) >= 1:
                inner_cv = inner_folds
                inner_desc = f"Block-StratifiedKFold(n_splits={len(inner_folds)})"
            else:
                inner_cv = None
        else:
            k_in = _safe_k_for_strat(y_tr, desired_inner)
            if k_in >= 2:
                inner_cv = StratifiedKFold(n_splits=k_in, shuffle=True, random_state=RNG)
                inner_desc = f"Window-StratifiedKFold(n_splits={k_in})"
            else:
                inner_cv = None
                inner_desc = "NO-InnerCV"

        # Optional: if you want SFS OFF for window-level inner CV, uncomment:
        # param_grid = dict(param_grid_common)
        # if isinstance(inner_cv, StratifiedKFold):
        #     param_grid["selector__sfs_k"] = [0]
        # else:
        #     param_grid = param_grid_common

        param_grid = param_grid_common

        # ---------------- Grid search on inner policy (or fit once) ----------------
        if inner_cv is None:
            best = clone(pipe)
            best.set_params(
                pca='passthrough',
                selector__k_best=max(2, default_kbest),
                selector__sfs_k=default_sfsk,
                selector__corr_thr=0.90,
                selector__C=1.0, selector__gamma='scale',
                svm__C=1.0, svm__gamma='scale',
            )
            best.fit(X_tr, y_tr, selector__feature_names=feature_names)
            last_best_params = {
                "pca": "passthrough",
                "selector__k_best": max(2, default_kbest),
                "selector__sfs_k": default_sfsk,
            ...
            gs = GridSearchCV(
                ...
                cv=inner_cv,
                n_jobs=-1, scoring="accuracy", refit=True, error_score="raise"
            )
            # If inner_cv is a list of (idx, idx), pass to selector so SFS uses same precomputed folds
            sfs_cv_param = inner_cv if (not isinstance(inner_cv, StratifiedKFold)) else None
            gs.fit(X_tr, y_tr,
                   selector__feature_names=feature_names,
                   selector__sfs_cv=None) # let SFS create local folds on current train slice
            best = gs.best_estimator_
            last_best_params = gs.best_params_

        fold_acc = float(best.score(X_te, y_te))
        outer_scores.append(fold_acc)

        print(f"[{label}] Fold {fold_id}/{len(folds_iter)} "
              f"| inner={inner_desc} | acc={fold_acc:.3f} | best={last_best_params}")
    ...
    # ---------------- Final refit with the same policy logic ----------------
    # Choose inner policy for ALL data
    if _blocks_per_class_ok(y, groups, 2):
        final_inner = make_block_stratified_folds(y, groups, desired_max_splits=3, strict_test_two_classes=True)
        refit_cv = final_inner
        refit_param_grid = param_grid_common
        refit_kwargs = {"selector__sfs_cv": final_inner}
        refit_desc = f"Block-StratifiedKFold(n_splits={len(final_inner)})"
    else:
        k_all = _safe_k_for_strat(y, 3)
        if k_all >= 2:
            refit_cv = StratifiedKFold(n_splits=k_all, shuffle=True, random_state=RNG)
            refit_param_grid = param_grid_common
            refit_kwargs = {"selector__sfs_cv": None}
            refit_desc = f"Window-StratifiedKFold(n_splits={k_all})"
        else:
            # fallback fit
            final_best = clone(pipe)
            final_best.set_params(
                pca='passthrough',
                selector__k_best=max(2, default_kbest),
                selector__sfs_k=default_sfsk,
                selector__corr_thr=0.90,
            ...
            final_best.fit(X, y, selector__feature_names=feature_names)
            try:
                sel = final_best.named_steps["selector"]
                sel_names = sel.selected_names_ if getattr(sel, "selected_names_", None) else []
            except Exception:
                sel_names = []
            print(f"[{label}] Final best params (no-CV): "
                  f"{{'pca':'passthrough','selector__k_best':{max(2, default_kbest)},"
                  f"'selector__sfs_k':{default_sfsk},'selector__corr_thr':0.90,"
                  f"'selector__C':1.0,'selector__gamma':'scale','svm__C':1.0,'svm__gamma':'scale'}}")
            print(f"[{label}] Selected features: {sel_names}")
            return final_best, mean_outer, {
                "pca": "passthrough",
                "selector__k_best": max(2, default_kbest),
                "selector__sfs_k": default_sfsk,
                "selector__corr_thr": 0.90,
    ...
    final_gs = GridSearchCV(
        estimator=pipe,
        param_grid=refit_param_grid,
        cv=refit_cv,
        n_jobs=-1, scoring="accuracy", refit=True, error_score="raise"
    )
    final_gs.fit(X, y, selector__feature_names=feature_names, selector__sfs_cv=None)
    ...
    print(f"[{label}] Final best params: {best_params} [{refit_desc}]")
    print(f"[{label}] Selected features: {sel_names}")
...
def extract_selected_indices(fitted_best: Pipeline) -> List[int]:
    try:
        steps = fitted_best.named_steps
        sel = steps.get("selector", None)
        pca_step = steps.get("pca", "passthrough")

        # If PCA is used, selector indices are in PCA space – don't slice originals.
        if pca_step != "passthrough":
            return list(range(steps["scaler"].n_features_in_))

        if sel is None:
            return list(range(steps["scaler"].n_features_in_))
        if getattr(sel, "selected_indices_", None):
            return sel.selected_indices_
        support_len = steps["scaler"].n_features_in_
...
def make_deployable_pipeline(
    ...
    if pca_step == "passthrough":
        # Safe: selector indices are in original space
        ...
        deploy.fit(X_train, y_all)
        return deploy
    else:
        # PCA used → train on all original features; no slicing
        deploy = _SkPipeline([
            ("scaler", StandardScaler()),
            ("pca", clone(pca_step)),
            ("svm", SVC(
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--start_activity", type=str, default="stand", choices=["", "sit", "stand", "walk", "turn"],
...
if __name__ == "__main__":
    ...
    # Determine expected input sizes for ONNX export:
    static_pca_used  = (static_pipe.named_steps.get("pca", "passthrough")  != "passthrough")
    dynamic_pca_used = (dynamic_pipe.named_steps.get("pca", "passthrough") != "passthrough")

    static_in_feats  = Xs.shape[1] if static_pca_used  else len(static_sel_idx)
    dynamic_in_feats = Xd.shape[1] if dynamic_pca_used else len(dynamic_sel_idx)

    export_onnx_deploy(static_deploy,  n_features=static_in_feats,  out_path=str(outdir / "static_svm.onnx"))
    export_onnx_deploy(dynamic_deploy, n_features=dynamic_in_feats, out_path=str(outdir / "dynamic_svm.onnx"))
